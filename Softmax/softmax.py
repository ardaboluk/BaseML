...
df_cm = pd.DataFrame(conf_mat, index = [i for i in range(0, test_labels.shape[0])], columns = [i for i in range(0, test_labels.shape[0])])
plt.figure()
...
for curClass, (preds, trues, plot_marker) in enumerate(zip(whole_predictions, whole_true_labels, itertools.cycle(['-','--','-.',':']))):
